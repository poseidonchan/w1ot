...
# Simplified ChemCPA model for OOD experiments
class SimplifiedChemCPA(nn.Module):
    ...
    def update_step(self, genes, condition_idx, optimize_adversary=True):
        ...
        if optimize_adversary:
            # Only update the adversary to better classify conditions
            loss = adv_loss
        else:
...
OUTPUT_DIR = "/fs/cbcb-scratch/cys/w1ot/test_baselines/ood_experiments/chemcpa"
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
NUM_EPOCHS = 500  # Reduced for faster training
...
def run_chemcpa_on_perturbation(dataset_name, pert_dir, pert_path):
    ...
    # Set latent dimension - for sciplex, we use 50
    LATENT_DIM = 50
    ...
    model = SimplifiedChemCPA(
        ...
        encoder_width=256,  # default from config
        ...
        reg_adversary=9.100951626404369,  # default from config
...
def main():
    """Main function to run ChemCPA on all OOD perturbations."""
    # Force flushing of print statements to ensure log contains output
    import sys
    import time
    
    print("Starting simplified ChemCPA OOD evaluation...", flush=True)
    ...
    # Run simplified ChemCPA on each OOD dataset
    results = []
